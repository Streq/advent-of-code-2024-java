...
public class Day6 {
  ...
  private static final String INPUT_PATH = inputPath(Day6.class);

  static World inputFromFile(String path) {
    return WorldReader.fromLines(Util.getFileAsLines(path));
  }
  ...
  static class Part1 {
    ...
    static long solve(World world) {
      var guard = world.guards().getFirst();
      var oldState = guard.state();

      boolean withinBounds = true;
      while (!guard.isLoopComplete() && withinBounds) {
        withinBounds = guard.advance(world);
        if (guard.state() != oldState) {
          System.out.println();
          System.out.println(String.join("\n", WorldReader.toLines(world)));
          System.out.println();
          oldState = guard.state();
        }
      }
      System.out.println();
      System.out.println(String.join("\n", WorldReader.toLines(world)));
      ...
      return guard.getTilesSteppedOn().size();
    }
  }
  ...
  static class Part2 {
    ...
    // Let's brute-force this piece of shit
    static long solve(World world) {
      var guard = world.guards().getFirst();

      while (guard.advance(world)) {
        if (guard.isLoopComplete()) {
          break;
        }
      }

      List<Point> tilesSteppedOn = guard.getTilesSteppedOn();
      Guard.Entry startingPosition = guard.history().getFirst();

      // skip first entry
      List<Point> tilesWhereBlockingDoesAnything = tilesSteppedOn.subList(1, tilesSteppedOn.size());

      var count = 0;
      for (Point point : tilesWhereBlockingDoesAnything) {
        world.mappedArea().setObstacle(point);
        if (isLoopy(world, startingPosition)) {
          ++count;
    ...
    private static boolean isLoopy(World world, Guard.Entry startingPosition) {
      var newGuard = Guard.from(startingPosition);
      while (newGuard.advance(world)) {
        if (newGuard.isLoopComplete()) {
          return true;
        }
      }
      return false;
